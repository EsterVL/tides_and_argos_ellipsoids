cross_gam_wrapper_par_ebb<-function(ndwi,  threshold = 0.16, mode=c('Ebb')){
#if (is.null(meters)) meters<-get0('meters')
  # y should be a vector with the tidelevels
# x is the the ndwi vector
length.out=1000
if (length(which(is.na(ndwi)))>0) {return(NA)} else {
...
#####
# now, if length of crossings > 1 e need to go for gam..
...
#######
# Now I have to check whether crossing of a thre are on the same side of the axis.
#CHECK WHAT THAT MEANS!!!
if (length(unique(sign(Up_crossings)))>1) {
# in this case I want to grab the 
...
cross_gam_wrapper_par_flow<-function(ndwi,  threshold = 0.16, mode=c('Flow')){
#if (is.null(meters)) meters<-get0('meters')
  # y should be a vector with the tidelevels
# x is the the ndwi vector
length.out=1000
